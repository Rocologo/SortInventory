...
public class SortInventory extends JavaPlugin {
	...
	private void loadConfigFile() {
		...
		File bccconfigfile = new File(PLUGIN_FOLDER,
				Config.CONFIG_FILE);
	...
	private void setupPermissions() {
		if (permissions3 || permissionsBukkit) {
			...
			if (permissions3Plugin == null && permissionsBukkitPlugin == null) {
				Messages
						.showInfo("Permissions3/PermissionsBukkit system not detected, defaulting to OP");
				return;
			} else {
			...
			if ((!sender.isOp()) && (sender instanceof Player)) {
				...
				if (permissionsBukkit) {
					if (p.hasPermission(PERMISSION_NODE + label)) {
						...
						//		+ "PermissionsBukkit: You dont have permission to: " +PERMISSION_NODE + label);
					} 
