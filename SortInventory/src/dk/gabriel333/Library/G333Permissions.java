...
public class G333Permissions {
	...
	public static Boolean permissionsBukkit = false; // PermissionsBukkit support
	...
	public static void setupPermissions(Plugin plugin) {
		...
		if (permissions3 || permissionsBukkit || permissionsex) {
			G333Messages.showWarning("Permissions3 or PermissionsBukkit is allready detected!");
			return;
		} else {
			...
			if (permissions3Plugin == null && permissionsBukkitPlugin == null && permissionsexPlugin==null) {
				G333Messages.showInfo("PermissionsBukkit/Permissions3/PermissionsEx system not detected, defaulting to OP");
				return;
			} else {
				...
				if (permissions3Plugin != null) {
					permission3Handler = ((Permissions) permissions3Plugin)
							.getHandler();
					permissions3 = true;
					G333Messages.showInfo("Permissions3/SuperpermBridge is detected. "
							+ ((Permissions) permissions3Plugin)
									.getDescription().getFullName());
					if (permissionsBukkit) {
						G333Messages.showInfo("OBS. If conflict in PermissionsBukkit and Permissions3/SuperPerm, PermissionsBukkit wins over Permissions/SuperPerm");

					}

				} 
				if (permissionsexPlugin != null) {
					//PermissionManager permissionsexManager = PermissionsEx.getPermissionManager();
					G333Messages.showInfo("PermissionsBukkit is detected.");
					if ( permissionsBukkit || permissions3){
						G333Messages.showWarning("It NOT recomended to use more that one permission system!");
		...
		if (permissions3 || permissionsBukkit || permissionsex) {
			if ((!sender.isOp()) && (sender instanceof Player)) {
				...
				if (permissionsBukkit) {
					...
					PermissionManager permissionsexManager = PermissionsEx.getPermissionManager();
					if(permissionsexManager.has(p, PERMISSION_NODE+label)){
					    return true; 
					    } else {
					    return false;
					    }
